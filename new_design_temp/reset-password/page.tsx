...
import { Eye, EyeOff, CheckCircle, Lock, Shield, ArrowLeft, ArrowRight } from "lucide-react";
		...
		<div className="min-h-screen bg-background font-sans text-white">
			...
			<header className="flex items-center justify-between border-b border-border px-6 md:px-10 py-4">
				<div className="flex items-center gap-4 text-secondary">
					...
					<h2 className="text-white text-xl font-bold tracking-tight">SkillSage</h2>
				</div>
			...
			<main className="flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
				<div className="w-full max-w-md">
					...
					{/* Password Form Card */}
					<div className="bg-surface-elevated border border-border shadow-2xl rounded-xl p-8">
						<form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
							...
							<div className="flex flex-col gap-2">
								...
								<div className="relative group">
									<input
										className="w-full bg-background border border-input text-white rounded-lg px-4 py-3.5 focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none transition-all placeholder:text-muted-foreground"
										placeholder="••••••••"
							...
							<div className="space-y-3">
								...
								<div className="grid grid-cols-3 gap-2 h-1.5">
									<div
										className={`rounded-full ${hasLength ? "bg-gradient-to-r from-primary to-accent" : "bg-border"}`}
									/>
									<div
										className={`rounded-full ${hasUpperLower ? "bg-gradient-to-r from-primary to-accent" : "bg-border"}`}
									/>
									<div
										className={`rounded-full ${hasNumberSymbol ? "bg-gradient-to-r from-primary to-accent" : "bg-border"}`}
									/>
								</div>
								<ul className="grid grid-cols-2 gap-y-2 mt-4">
									<li className="flex items-center gap-2 text-xs text-muted-foreground">
										<CheckCircle
											className={`size-3.5 ${hasLength ? "text-secondary" : "text-border"}`}
										/>
										8+ characters
									</li>
									<li className="flex items-center gap-2 text-xs text-muted-foreground">
										<CheckCircle
											className={`size-3.5 ${hasUpperLower ? "text-secondary" : "text-border"}`}
										/>
										Lowercase &amp; Uppercase
									</li>
									<li className="flex items-center gap-2 text-xs text-muted-foreground">
										<CheckCircle
											className={`size-3.5 ${hasNumberSymbol ? "text-secondary" : "text-border"}`}
										/>
										Numbers &amp; Symbols
									</li>
									<li className="flex items-center gap-2 text-xs text-muted-foreground">
										<CheckCircle
											className={`size-3.5 ${isUnique ? "text-secondary" : "text-border"}`}
										/>
										Unique from previous
									</li>
								</ul>
							</div>
							...
							{/* Submit Button */}
							<button
								className="w-full bg-gradient-to-r from-primary to-accent text-white font-bold py-4 rounded-lg shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all flex items-center justify-center gap-2 group"
								type="submit"
							>
								Update Password
